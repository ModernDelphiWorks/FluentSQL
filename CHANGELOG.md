...
- Três testes em `Test Delphi\Common_tests\test.core.params.pas` travando a parametrização do slot de valor de `SetValue`/`Values(array of const)` — que até então não tinha teste nenhum. Rodam nos projetos **Firebird** e **MySQL**, os dois que compilam esse arquivo.
