...
- **BREAKING CHANGE (SQL emitido) — `IFluentSQLMerge.Update`/`.Insert` passaram a parametrizar valores string.** Valores numéricos já viravam `:pN`; **strings iam verbatim para o texto do SQL**, sem aspas e sem escape. Quem compara o SQL gerado com string fixa **precisa atualizar as expectativas**:
  ...
  **Isto não era só um buraco de segurança latente — era funcionalidade quebrada.** O caso benigno já não funcionava em motor nenhum: `SET [NOME] = TESTE` é `Msg 207, Invalid column name 'TESTE'` no SQL Server, e `O'Brien` dava `Msg 105, Unclosed quotation mark`. Com valor hostil, era injeção executável: medido em SQL Server 2022, `.Update(['NOME', '1; DROP TABLE USERS; --'])` **derrubou a tabela**. Depois da correção o mesmo payload chega ao banco como dado da coluna. Oráculo completo, com `docker run`, versão do motor e saída bruta antes e depois: `Test Delphi\Common_tests\test.merge.mssql.sql`.
  ...
  A regra é a que o overload tipado `SetValue(const AColumnName, AColumnValue: String)` já seguia: no array de `.Update`/`.Insert` os slots ímpares são **nomes de coluna** (identificadores, seguem literais) e os pares são **valores** (sempre `:pN`). Isso difere de `Where`, `Column`, `Having` e `Merge.On`, onde `array of const` está em posição de **expressão** e string continua sendo fragmento de SQL — **essas não mudaram**.
...
- Matriz `Test Delphi\Common_tests\test.merge.matrix.pas` (9 testes): uma célula por dialeto, afirmando parametrização onde há serializador e **exceção nomeada** onde não há. `Test Delphi\MSSQL_tests\test.merge.mssql.pas` foi de 5 para **27** testes — inclui 16 casos de payload hostil (aspa simples, aspa dupla, `;`, `--`, `/* */`, aspa dobrada, `UNION`), o `O'Brien` que **tem** de funcionar, e a guarda de lista malformada.
...
- **`MERGE` com lista de pares de contagem ímpar emitia SQL inválido em silêncio.** `.Insert(['ID', 1, 'NOME'])` gerava `VALUES (:p1, )` e `.Update(['NOME'])` gerava `SET [NOME] = `; o SQL Server recusa ambos com `Msg 102`. Um nome sem valor não tem serialização possível — passou a levantar `EArgumentException` na chamada, com a contagem na mensagem, em vez de falhar no motor do consumidor longe da linha que causou. Contagem par, inclusive a lista vazia, continua passando.
...
- **`array of const` em posição de expressão continua interpolando string verbatim.** Vale para `Where`, `Column`, `Having`, `OnCond`, `CaseExpr` e `Merge.On`: escalares numéricos viram `:pN`, strings seguem literais, por serem tratadas como fragmento de SQL (identificador, operador, trecho). `.On(['t.NOME =', 'x''; DROP...'])` emite `ON (t.NOME = x'; DROP...)`. **Isto não mudou nesta entrega e é intencional** — só `.Update`/`.Insert` do `MERGE` mudaram, porque ali o array é estritamente uma lista de pares nome/valor e o slot par não tem como ser fragmento. A distinção entre **valor** e **expressão** que separa os dois casos está sob revisão como tarefa própria.
