...
- **BREAKING CHANGE (SQL emitido) — `SetValue(nome, array of const)` e `Values(nome, array of const)` passaram a parametrizar valores string.** É o mesmo defeito da entrada acima, no `INSERT`/`UPDATE` comum em vez do `MERGE`, e a régua é a mesma: string deixou de ir verbatim e passou a `:pN`. Quem compara o SQL gerado com string fixa **precisa atualizar as expectativas**:
  ...
  **`SetValue`/`Values` também passaram a levantar `EArgumentException` em três formas que antes saíam caladas** — é a entrada irmã da do `MERGE`, logo abaixo, e vale para o `INSERT`/`UPDATE` comum, que é o caminho **mais** trafegado dos dois:
  ...
  **O `nil` não é novidade desta rodada, mas nunca tinha sido declarado nem testado neste caminho.** O ramo `vtPointer` de `TUtils._StringVarRecAsParam` (`FluentSQL.Utils.pas:184-188`) é **compartilhado** pelo `MERGE` e por `SetValue`/`Values`, então a guarda introduzida para o `MERGE` já valia aqui desde então — mas a entrada do CHANGELOG falava só em "chamadas de `MERGE`", e a mutação daquele ramo só derrubava testes de `MERGE`: a guarda do caminho mais usado estava **sem oráculo nenhum**. Agora tem, em `Test Delphi\Common_tests\test.core.params.pas`. Que `nil` devesse virar `NULL` em vez de levantar continua sendo **decisão de convenção não tomada** — os testes travam o comportamento atual, não o abençoam.

  **As outras duas são guardas novas**, e existem porque `CHANGELOG` afirmava que "a régua é a mesma" do `MERGE` enquanto o irmão seguia emitindo SQL inválido em silêncio. A de mais de um valor é a pior das duas porque **parece** correta: tem parâmetros de verdade e `Params.Count = 2`. Medido em execução real antes de decidir a forma, seis motores, **zero aceitam** qualquer das duas formas: MSSQL 2022 `Msg 102`; PostgreSQL 16.14 `syntax error at or near ")"` / `near "$2"`; Oracle Free 23 `ORA-00936` / `ORA-01745`; Firebird 5.0.4 `-104 Token unknown ')'` / `'?'`; MySQL 8.4.11 `ERROR 1136` / `ERROR 1064`; SQLite 3.53.4 `Parse error near ")"` / `near "?"`. Controle acompanhando: `VALUES (?, ?)` **com** vírgula é aceito nos seis — logo a recusa é da forma, não do `INSERT`. Saída bruta e `docker run` em `Test Delphi\Common_tests\test.setvalue.mssql.sql`.
  ...
  **O que fazer (parametrização):** se você dependia de passar fragmento de SQL por esse array — por exemplo `.SetValue('DATA', ['CURRENT_TIMESTAMP'])` — ele agora vira **dado**, e a coluna recebe a string `CURRENT_TIMESTAMP`. **Não há substituto hoje:** o overload tipado `SetValue(const AColumnName, AColumnValue: String)` também parametriza (`FluentSQL.pas:411`), e sempre parametrizou. Ou seja, o `INSERT`/`UPDATE` do FluentSQL **não exprime expressão em slot de valor** — nem antes nem depois; o que existia era um caminho que funcionava *por acidente*, e apenas quando o texto passado calhava de ser SQL válido no dialeto alvo. Isso está registrado em *Known issues* como a distinção **valor × expressão**, que é tarefa de arquitetura própria.

- **BREAKING CHANGE (comportamento) — seis formas de `MERGE` que não levantavam nada passaram a levantar `EArgumentException`.** Todas as seis emitiam SQL que **nenhum motor executa**, e o faziam em silêncio: o erro só aparecia no banco do consumidor, longe da linha que o causou. *(A tabela abaixo é a lista completa; uma versão anterior desta entrada dizia "quatro" e já listava cinco linhas.)*
  ...
  **Consequência de ordem, declarada de propósito:** como essa guarda roda **antes** do despacho por dialeto, um `MERGE` sem `WHEN` montado sobre um dialeto sem serializador (MySQL, SQLite, …) recebe `EArgumentException` e **não** `EFluentSQLStatementNotSupported`. As duas seriam verdadeiras; a escolhida é a que aponta a linha que o consumidor escreveu.

  O caso do **`nil` é de natureza diferente e pior**: não emitia SQL inválido, emitia SQL **válido com o dado errado**. `nil` num `array of const` chega como `vtPointer` e era convertido por `IntToHex`, então a coluna recebia a string `'00000000'` — corrupção silenciosa, sem erro em lugar nenhum. O par nome/valor não tem como exprimir `NULL`, e dar essa semântica ao `nil` é decisão de convenção que **não** foi tomada aqui.
