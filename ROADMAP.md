...
- **DDL — próxima vertical planeada (pipeline):** **ESP-029** — **`TRUNCATE TABLE`** (**ADR-029**); artefactos em `.claude/pipeline/`; issue GitHub a criar no **`/task`**. **ESP-028** ([#43](https://github.com/ModernDelphiWorks/FluentSQL/issues/43)) — implementação + QA no repositório; fecho versionado típico **`/develop`** / **`/release`**. **ESP-025** … **ESP-027** (`DROP INDEX` / **`IF EXISTS`** / **`CONCURRENTLY`**): implementação no repositório; issues [#40](https://github.com/ModernDelphiWorks/FluentSQL/issues/40), [#41](https://github.com/ModernDelphiWorks/FluentSQL/issues/41), [#42](https://github.com/ModernDelphiWorks/FluentSQL/issues/42); fechos versionados e checkboxes do roadmap via **`/develop`** / **`/release`** conforme política.
...
- [ ] **ESP-029** — DDL alargado (9ª vertical): **`TRUNCATE TABLE`** (API fluente + **PostgreSQL** / **Firebird** / **MySQL**, **ADR-029**); testes em `test.ddl.pas` e guia em `docs-src`; *ver `.claude/pipeline/esp.md` / `plan.md`.*
- [ ] DDL Fluente (âmbito alargado): `ALTER TABLE` (além de ADD/DROP column), restantes índices/alterações e extensões ainda não cobertas por ESP dedicadas (avaliar portabilidade; o que for estritamente específico de um motor → **ESP-016** ou app). *Nota: `DROP TABLE` — **ESP-018**; **ADD COLUMN** — **ESP-019**; **DROP COLUMN** — **ESP-020**; **CREATE INDEX** — **ESP-022**; **DROP INDEX** — **ESP-025**; **`IF EXISTS` em `DROP INDEX`** — **ESP-026**; **`CONCURRENTLY` (PG)** — **ESP-027**; **`ON table` (MySQL)** — **ESP-028**; **`TRUNCATE TABLE`** — **ESP-029**; esta linha cobre o restante alargamento.*
