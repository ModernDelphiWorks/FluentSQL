...
- **Extensão por motor (ESP-016, ADR-016):** **`ForDialectOnly`** regista sufixos ou fragmentos gerados a partir de **`array of const`** que só entram na SQL final quando o dialeto da instância coincide; noutros motores o trecho é omitido. Ver o guia [Extensão explícita por motor](guides/extensao-por-dialeto.md) e a issue [#27](https://github.com/ModernDelphiWorks/FluentSQL/issues/27).
