...
> **Release alignment:** examples and issue pointers below are reconciled with **`CHANGELOG.md`** and tag **v1.1.0** (2026-04-09).
...
- **DDL (CREATE / DROP / ALTER ADD COLUMN, v1.1.0):** **`Test Delphi/test.ddl.pas`** — `TTestDDLCreateTable` (**ESP-017**), `TTestDDLDropTable` (**ESP-018** / **ADR-018**), `TTestDDLAlterTableAddColumn` (**ESP-019** / **ADR-019**); referenciado em **`Firebird_tests/PTestFluentSQLFirebird.dpr`**. Evidência de execução DUnitX pode depender do ambiente MSBuild/Delphi (ver issue [#32](https://github.com/ModernDelphiWorks/FluentSQL/issues/32)).
