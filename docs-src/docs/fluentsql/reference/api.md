...
| DDL (`DROP INDEX`, ESP-025 / ESP-026 / ESP-027) | **`CreateFluentDDLDropIndex(ADialect, 'INDICE')`** devolve **`IFluentDDLDropIndexBuilder`**; **`AsString`** gera **`DROP INDEX`**; opcional **`.IfExists`** para **`DROP INDEX IF EXISTS`** (**ADR-026**); opcional **`.Concurrently`** só em **PostgreSQL** para **`DROP INDEX CONCURRENTLY`** (**ADR-027**). Ver [DDL — DROP INDEX](../guides/ddl-drop-index.md). |
...
- **DDL (v1.1.0: ESP-017–ESP-019; v1.1.1: ESP-020 / ESP-022):** builders em `FluentSQL.DDL.pas` e serializers em `FluentSQL.DDL.Serialize.pas` emitem **apenas strings**; o FluentSQL não executa DDL. Dialetos não suportados levantam **`ENotSupportedException`** com mensagem estável referindo o âmbito da entrega (**ESP-017**–**ESP-022**).
