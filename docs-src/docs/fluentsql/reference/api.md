...
- **Extensão por motor (ESP-016, ADR-016):** **`ForDialectOnly`** acrescenta fragmentos **opt-in** associados a um **`TFluentSQLDriver`**; só entram na string final quando o dialeto da instância coincide. Não substitui o núcleo portável por recursos “modernos” universais. Ver o guia [Extensão explícita por motor](../guides/extensao-por-dialeto.md).
