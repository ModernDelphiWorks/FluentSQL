...
| DDL (`DROP INDEX`, ESP-025 / ESP-026) | **`CreateFluentDDLDropIndex(ADialect, 'INDICE')`** devolve **`IFluentDDLDropIndexBuilder`**; **`AsString`** gera **`DROP INDEX`**; opcional **`.IfExists`** para **`DROP INDEX IF EXISTS`** (ver **ADR-026**). Ver [DDL — DROP INDEX](../guides/ddl-drop-index.md). |
